...
# [rc4l] Pin the bootstrapped vcpkg to a known-good release. Unpinned HEAD pulls ffmpeg 8.1.2,
# which does NOT build with MSVC (D3D12 ID3D12Resource::GetDesc ABI mismatch -> C2197, and the
# new libswscale ops scaler needs <stdalign.h>, absent from MSVC). 2025.12.12 still ships the
# MSVC-clean ffmpeg 7.1.2. CI is unaffected -- it uses the runner's own pinned vcpkg, not this clone.
$VcpkgTag   = "2025.12.12"
...
function Resolve-Vcpkg {
    ...
    if (-not (Test-Path (Join-Path $local "vcpkg.exe"))) {
        ...
        if (-not (Test-Path (Join-Path $local ".git"))) {
            & git clone --depth 1 --branch $VcpkgTag https://github.com/microsoft/vcpkg.git $local
            if ($LASTEXITCODE -ne 0) { throw "git clone of vcpkg failed" }
        } else {
            # [rc4l] Existing clone (e.g. bootstrap was interrupted): make sure it is on the pinned tag.
            & git -C $local rev-parse -q --verify "refs/tags/$VcpkgTag" *> $null
            if ($LASTEXITCODE -ne 0) {
                & git -C $local fetch --depth 1 origin "refs/tags/${VcpkgTag}:refs/tags/${VcpkgTag}"
                if ($LASTEXITCODE -ne 0) { throw "git fetch of vcpkg tag $VcpkgTag failed" }
            }
            & git -C $local -c advice.detachedHead=false checkout $VcpkgTag
            if ($LASTEXITCODE -ne 0) { throw "git checkout of vcpkg tag $VcpkgTag failed" }
        }
...
# (libav*) pulls in on Windows for its Media Foundation + networking + DirectShow glue. shlwapi
# resolves mpg123 compat.c's PathCombineW/PathIsRelativeW/PathIsUNCW (the Shell path helpers).
