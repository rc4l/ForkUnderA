...
100% line coverage proves a line ran, not that it ran with a value that breaks it. When a `Compute*` helper is arithmetic over scalars and you can state a rule about it — two implementations that must agree, a round-trip that must be the identity, an invariant that must always hold — write a colocated `*_fuzz.cpp` property harness alongside the `*_test.cpp`. See the `writing-tests` skill and `tests/fuzz.sh`.
