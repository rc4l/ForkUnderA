...
static int resampler_run_blam(resampler * r, float ** out_, float * out_end)
{
    ...
    if ( in_size > 0 )
    {
        ...
        do
        {
            ...
            sample = in[0];
            if (phase_inc.quad < 0x100000000ll)
                // [rc4l] Was `* phase`, which does not compile -- phase is a bigint union.
                // Upstream's HEAD spells this `* phase.quad`, but the x86 and generic
                // siblings of this same BLAM resampler (see the matching lines above) scale
                // by 1/2^32, and they are right: phase is a 32.32 fixed-point position, so
                // without the scale this interpolation weight reaches ~2^32 instead of
                // staying in [0,1). Matched to the siblings rather than to upstream's ARM.
                sample += (in[1] - in[0]) * phase.quad * (1.f / 0x100000000ll);
            sample -= last_amp;
