...
extern "C" bool Mac_HttpsGet(const char* urlStr, char* out, int outSize, int timeoutSecs)
{
	...
	@autoreleasepool
	{
		...
		// Overall cap a couple seconds past the request timeout, so we never block forever even if the
		// completion handler is delayed.
		dispatch_time_t deadline = dispatch_time( DISPATCH_TIME_NOW, (int64_t)( timeoutSecs + 2 ) * NSEC_PER_SEC );
		if ( dispatch_semaphore_wait( sem, deadline ) != 0 )
			return false; // timed out waiting for the handler
