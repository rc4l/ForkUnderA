...
*/			}
	...
	void DrawPopScreen (int bottom)
	{
		...
		int height = clamp<int> ((const int)(PopHeight + FixedMul (r_TicFrac, PopHeightChange)), -POP_HEIGHT, 0);
			...
			// declaration cannot be jumped over by the other case labels.
		{
			...
			if (CPlayer->LogText.IsNotEmpty())	// [rc4l] uzdoom@4aac586dd -- FString, so != NULL built a temporary
			{
		...
		case POP_Keys:
			...
			if (KeyPopScroll > 0)
			{
				...
				int scroll = MAX<int> (0,(const int)(KeyPopScroll - FixedMul (r_TicFrac, 280/KEY_TIME)));
				pos -= 10;
