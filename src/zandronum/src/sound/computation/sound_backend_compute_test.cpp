...
// [rc4l] SoundBackendReinitNeeded -- the double-init fix. The snd_backend callback used to
// unconditionally close+reopen the sound device; on startup that fired once (from
// EnableCallbacks) right after I_Init had already built the renderer, so the device was
// opened, closed, and reopened -- the doubled "Initializing OpenAL" log. This decides when
// the callback may skip that redundant rebuild.
namespace
{
bool ReinitNeeded(int active, const char *requested, bool noSound = false,
	bool present = true)
{
	return SoundBackendReinitNeeded(active, requested, noSound, /*compiledIn=*/true, present);
}
} // namespace

TEST(SoundBackendReinit, SkipsWhenActiveOpenALMatchesRequest)
{
	// [rc4l] THE startup case: I_Init built OpenAL, then EnableCallbacks fires this with the
	// same backend -> nothing to do. (Also the wad_reload restart case: backend unchanged.)
	EXPECT_FALSE(ReinitNeeded(ZX_SNDBACKEND_OPENAL, "openal"));
}

TEST(SoundBackendReinit, SkipsWhenActiveNullMatchesNullRequest)
{
	EXPECT_FALSE(ReinitNeeded(ZX_SNDBACKEND_NULL, "null"));
}

TEST(SoundBackendReinit, RebuildsWhenNothingIsActiveYet)
{
	// [rc4l] Before I_InitSound has ever run, the tracker is NONE -> must build.
	EXPECT_TRUE(ReinitNeeded(ZX_SNDBACKEND_NONE, "openal"));
	EXPECT_TRUE(ReinitNeeded(ZX_SNDBACKEND_NONE, "null"));
}

TEST(SoundBackendReinit, RebuildsOnAGenuineRuntimeSwitch)
{
	// [rc4l] The feature the CUSTOM_CVAR exists for -- switching backend live still rebuilds.
	EXPECT_TRUE(ReinitNeeded(ZX_SNDBACKEND_OPENAL, "null"));
	EXPECT_TRUE(ReinitNeeded(ZX_SNDBACKEND_NULL, "openal"));
}

TEST(SoundBackendReinit, NormalizesToTheRendererActuallyConstructed)
{
	// [rc4l] An unavailable OpenAL and an unknown name both run the NULL renderer, so against
	// an already-null active backend they are a no-op -- comparing the raw choice enum would
	// wrongly rebuild.
	EXPECT_FALSE(ReinitNeeded(ZX_SNDBACKEND_NULL, "openal", /*noSound=*/false, /*present=*/false));
	EXPECT_FALSE(ReinitNeeded(ZX_SNDBACKEND_NULL, "wasapi"));
}

TEST(SoundBackendReinit, NoSoundServerResolvesToNull)
{
	// [rc4l] -nosound/-host forces the null renderer regardless of the cvar name.
	EXPECT_FALSE(ReinitNeeded(ZX_SNDBACKEND_NULL, "openal", /*noSound=*/true));
	EXPECT_TRUE(ReinitNeeded(ZX_SNDBACKEND_OPENAL, "openal", /*noSound=*/true));
}
