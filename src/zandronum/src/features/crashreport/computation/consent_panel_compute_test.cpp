// [rc4l] Tests for the crash-consent panel geometry/gradient helper. Exercises every line
// (the coverage gate enforces 100% on *_compute.cpp).
...
TEST(ConsentPanelRect, CentresAndWrapsContent)
{
	// 640x480 screen, 300-wide panel, content rows [100,300], padY 12.
	ConsentPanelRect r = ComputeConsentPanelRect(640, 480, 300, 100, 300, 12, 10);
	EXPECT_EQ(r.w, 300);
...
TEST(ConsentPanelRect, WidthClampedToScreen)
{
	// panelW wider than the screen -> ClampInt high branch.
	ConsentPanelRect r = ComputeConsentPanelRect(400, 400, 999, 50, 150, 8, 6);
	EXPECT_EQ(r.w, 400);
	EXPECT_EQ(r.x, 0);
}

TEST(ConsentPanelRect, NegativeWidthClampedToZero)
{
	// panelW negative -> ClampInt low branch.
	ConsentPanelRect r = ComputeConsentPanelRect(400, 400, -50, 50, 150, 8, 6);
	EXPECT_EQ(r.w, 0);
}

TEST(ConsentPanelRect, ClampsTopBottomAndRadius)
{
	// contentTop-padY < 0 -> top=0 ; contentBottom+padY > screenH -> bottom=screenH.
	ConsentPanelRect r = ComputeConsentPanelRect(200, 120, 180, 5, 200, 10, 999);
...
TEST(ConsentPanelRect, DegenerateHeightAndScreen)
{
	// contentBottom+padY < contentTop-padY -> h < 0 -> h=0 ; screenW <= 0 -> maxW=0 -> w=0.
	ConsentPanelRect r = ComputeConsentPanelRect(0, 100, 50, 90, 10, 0, 4);
	EXPECT_EQ(r.w, 0);
	EXPECT_EQ(r.h, 0);
...
{
	ConsentPanelColor top = { 100, 60, 20, 240 };
	ConsentPanelColor bot = { 0, 0, 0, 200 };

	ConsentPanelColor a = ComputeConsentGradient(0, 101, top, bot);
	EXPECT_EQ(a.r, 100);
	EXPECT_EQ(a.a, 240);

	ConsentPanelColor z = ComputeConsentGradient(100, 101, top, bot);
	EXPECT_EQ(z.r, 0);
	EXPECT_EQ(z.a, 200);

	ConsentPanelColor m = ComputeConsentGradient(50, 101, top, bot);
...
{
	ConsentPanelColor top = { 30, 30, 30, 255 };
	ConsentPanelColor bot = { 10, 10, 10, 100 };

	// height <= 1 -> den forced to 1; row 0 yields the top colour.
	ConsentPanelColor one = ComputeConsentGradient(0, 1, top, bot);
	EXPECT_EQ(one.r, 30);
	EXPECT_EQ(one.a, 255);

	// row beyond range is clamped to the last row (bottom colour).
	ConsentPanelColor over = ComputeConsentGradient(999, 5, top, bot);
	EXPECT_EQ(over.r, 10);
	EXPECT_EQ(over.a, 100);

	// negative row clamps to the top colour.
	ConsentPanelColor under = ComputeConsentGradient(-4, 5, top, bot);
