...
TEST(IwadSubstitute, StandsInForABaseFileWeMayNotRedistribute)
{
	// megagame.wad is freely given away but carries no licence anyone can cite, so it is not ours
	// to ship. fuamega.wad is generated by tools/mkiwad to take its place, and the two are
	// interchangeable in play: the join check hashes the current map's lumps, and in a total
	// conversion those come from the pk3 rather than from the base file.
	EXPECT_EQ("fuamega.wad", FreeIwadSubstituteFor("megagame.wad"));

	// Ours is a destination, never a source. Substituting for it would swap away the one file we
	// know every install has.
	EXPECT_EQ("", FreeIwadSubstituteFor("fuamega.wad"));
}
