...
#include "features/levelmesh/projdecals.h"   // [rc4l] the marks this pass draws
...
// granularities and only one of them is safe. On the mesh path the fade is constant across each
// surface, so a corner takes two different constants and shows a hard step down the join -- which
// is why it defaults off there. Here it is per FRAGMENT, from the same normal buffer the surface
// wrote, so it varies continuously across a corner and there is no seam to make.
//
...
static int g_ddDrawn = 0, g_ddBoxes = 0, g_ddTextures = 0, g_ddTexSlots = 0;
static const char *g_ddBail = "";

void GetDeferredDecalStats(int &boxes, int &draws, int &textures, int &slots, const char **bail)
{
	boxes = g_ddBoxes; draws = g_ddDrawn; textures = g_ddTextures; slots = g_ddTexSlots;
...
static bool EnsureDeferredDecalPass()
{
	...
	// [rc4l] What the shader compiler actually made of the texture ARRAY, asked rather than assumed.
	//
	// An array that reflects as a single element takes the first texture and gives every mark in the
	// level the same graphic -- and since an unbound slot falls back to the white texture, "every
	// mark is a solid rectangle" is what that looks like on screen. It is indistinguishable from a
	// dozen other faults until the number is printed.
	{
		g_ddTexSlots = 0;
		if (auto *v = g_ddSRB->GetVariableByName(Diligent::SHADER_TYPE_PIXEL, "uDecalTex"))
		{
			Diligent::ShaderResourceDesc rd;
			v->GetResourceDesc(rd);
			g_ddTexSlots = (int)rd.ArraySize;
		}
	}
	return true;
}
...
// sprite in the room.
static void DrawDeferredDecals(Diligent::IDeviceContext *ctx)
{
	g_ddDrawn = 0;
	g_ddBoxes = 0;
	g_ddTextures = 0;
	...
	float invMVP[16];
	if (!InvertMatrix4(g_mvp, invMVP)) { g_ddBail = "view matrix not invertible"; return; }
	{
...
static void DrawWorld(Diligent::IDeviceContext *ctx)
{
	...
	if (fua_decalmode == 2) DrawDeferredDecals(ctx);
