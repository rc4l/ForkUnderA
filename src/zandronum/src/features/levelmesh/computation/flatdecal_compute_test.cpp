...
TEST(FlatDecalCompute, AStationarySurfaceKeepsTheDecalWhereItLanded)
{
	...
	EXPECT_FLOAT_EQ(ComputeDecalHeight(192.f, 0.f, 0.f, false, 0.05f), 192.05f);
}
...
TEST(FlatDecalCompute, ThePlanesOwnHeightIsNeverUsedDirectly)
{
	...
	EXPECT_FLOAT_EQ(ComputeDecalHeight(192.f, 0.f, 0.f, false, 0.f), 192.f);
	EXPECT_FLOAT_EQ(ComputeDecalHeight(192.f, 64.f, 64.f, false, 0.f), 192.f);
	EXPECT_FLOAT_EQ(ComputeDecalHeight(192.f, -300.f, -300.f, false, 0.f), 192.f);
}

TEST(FlatDecalCompute, ARisingLiftCarriesItsDecalsUp)
{
	// Shot at 64 on a platform that has since risen 40 units.
	EXPECT_FLOAT_EQ(ComputeDecalHeight(64.f, 0.f, 40.f, false, 0.f), 104.f);
}

TEST(FlatDecalCompute, ADescendingLiftCarriesThemDown)
{
	EXPECT_FLOAT_EQ(ComputeDecalHeight(64.f, 40.f, 0.f, false, 0.f), 24.f);
}
...
TEST(FlatDecalCompute, ACeilingDecalIsOffsetDownwardsIntoTheRoom)
{
	...
	EXPECT_FLOAT_EQ(ComputeDecalHeight(128.f, 0.f, 0.f, true, 0.05f), 127.95f);
}

TEST(FlatDecalCompute, TheOffsetGoesOppositeWaysForTheTwoSides)
{
	const float floorZ = ComputeDecalHeight(100.f, 0.f, 0.f, false, 2.f);
	const float ceilZ  = ComputeDecalHeight(100.f, 0.f, 0.f, true, 2.f);
...
{
	// Both terms at once, on a ceiling that has descended 10 units.
	EXPECT_FLOAT_EQ(ComputeDecalHeight(128.f, 200.f, 190.f, true, 0.05f), 117.95f);
}
